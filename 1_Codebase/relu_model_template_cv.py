...
# NEW: use your class file and folder of matrices
...
def build_cnn_bilstm_model(
    ...
    # ----- 1 x BiLSTM block: LSTM_HIDDEN_SIZE units per direction -----
    x = layers.Bidirectional(
        layers.LSTM(
            LSTM_HIDDEN_SIZE,
            return_sequences=False,	  # final vector per protein
            dropout=0.0,                  # do not apply input dropout inside LSTM
            unit_forget_bias=True,
        )
    )(x)  # shape: (batch, 1 * LSTM_HIDDEN_SIZE) = (batch, 256)
